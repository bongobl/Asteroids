...
public class FlyingSaucer extends SpaceObject{
	...
	public FlyingSaucer(int type, int direction, Player player, ArrayList<Bullet> bullets){
		...
		y = MathUtils.random(MainClass.HEIGHT);
		shapex = new float[6];
		shapey = new float[6];
		setShape();
		if(type == LARGE){
	...
	public void update(float dt){
		...
		if(!player.isHit()){
			...
			if(fireTimer > fireTime){
				...
				if(type == LARGE){
					radians = MathUtils.random(2 * PI);
				}else if(type == SMALL){
					radians = MathUtils.atan2(player.gety() - this.y, player.getx() - this.x);	
		...
		if(pathTimer > pathTime1 && pathTimer < pathTime2){
			dy = -speed * MathUtils.sin(PI / 4);
			dx = speed * direction * MathUtils.cos(PI / 4);
		...
		//screen wrap
		if(y < 0) y = MainClass.HEIGHT;
		
		//setShape
		setShape();
		
		if((direction == RIGHT && x > MainClass.WIDTH) || (direction == LEFT && x < 0)){
			remove = true;
		}
	}
